...
def _recompute_activity_totals_from_efforts(activity_id: int):
    ...
    row = cache._conn.execute(
        "SELECT gpx_points, stream_length, moving_time_s FROM activities WHERE activity_id=?",
        (activity_id,)
    ).fetchone()
    ...
    # Dislivello: evita doppio conteggio su segmenti sovrapposti.
    # Strategia più stabile: applica prima i segmenti più lunghi.
    elev_sum = 0.0
    used = []
    elev_candidates = []
    for si, ei, e in idx_intervals:
        ...
        if elev <= 0:
            continue
        seg_len = max(1, ei - si)
        elev_candidates.append((seg_len, si, ei, elev))

    for seg_len, si, ei, elev in sorted(elev_candidates, key=lambda x: x[0], reverse=True):
        uncovered = seg_len
        for usi, uei in used:
    ...
    # Guard-rail anti outlier (merge/indice sporco): evita valori fisicamente implausibili.
    mt = float(row["moving_time_s"] or 0.0) if row else 0.0
    caps = []
    if covered_distance > 0:
        caps.append(covered_distance * 0.06)  # 6% medio su tutta l'uscita
    if mt > 0:
        caps.append((mt / 3600.0) * 1800.0)  # VAM medio molto alto ma realistico
    if caps:
        elev_sum = min(elev_sum, max(caps))
...
def get_cache():
    global _cache
    if _cache is None:
        _cache = SegmentCache(str(resolve_cache_db_path(_config.get("cache", {}))))
...
def api_refresh_activity(activity_id):
    ...
    # Il ricalcolo locale deve funzionare sempre, anche se Strava è irraggiungibile.
    ...
    try:
        ...
        if "429" in err or "Rate Limit" in err or "Too Many Requests" in err:
            logger.warning("⏱ Refresh bloccato: rate limit Strava — applicato solo ricalcolo locale")
            return jsonify({
                "ok": True,
                "saved": 0,
                "local_only": True,
                "warning": "⏱ Rate limit Strava: applicato solo ricalcolo locale",
            })
        logger.error(f"Refresh attività fallito lato Strava (activity_id={activity_id}): {e}. Applicato solo ricalcolo locale")
        return jsonify({
            "ok": True,
            "saved": 0,
            "local_only": True,
            "warning": f"Refresh Strava fallito: {err}",
        })
